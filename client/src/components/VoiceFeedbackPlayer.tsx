...
export const VoiceFeedbackPlayer: React.FC<VoiceFeedbackPlayerProps> = ({
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-indigo-950/80 via-slate-900/95 to-slate-950 border border-indigo-500/30 p-4 shadow-xl backdrop-blur-md">
      {/* Glow Effect */}
        ...
        <div className="flex items-center space-x-3">
          <div className="relative">
            <img
              src={avatarUrl}
              alt={coachName}
              className="w-12 h-12 rounded-2xl object-cover border-2 border-indigo-400/60 shadow-lg shadow-indigo-500/20"
            />
            {isPlaying ? (
              <span className="absolute -bottom-1 -right-1 w-3.5 h-3.5 rounded-full bg-emerald-400 ring-2 ring-slate-950 animate-ping" />
            ) : (
              <span className="absolute -bottom-1 -right-1 w-3.5 h-3.5 rounded-full bg-indigo-500 ring-2 ring-slate-950 flex items-center justify-center text-[8px] text-white font-bold">
          ...
          <div>
            <div className="flex items-center space-x-1.5">
              ...
              <span className="bg-gradient-to-r from-indigo-500/20 to-cyan-500/20 text-indigo-300 text-[10px] font-bold px-2 py-0.5 rounded-full border border-indigo-500/30 flex items-center space-x-1">
        ...
        <div className="flex items-center space-x-2">
          ...
          <button
            ...
            className={`w-11 h-11 rounded-2xl flex items-center justify-center font-bold text-white transition-all shadow-lg ${
              isPlaying
                ? 'bg-amber-500 hover:bg-amber-400 shadow-amber-500/30 scale-105 animate-pulse'
      ...
      <div className="mt-3.5 flex items-center space-x-2 py-2 px-3 rounded-2xl bg-slate-950/70 border border-slate-800/90 relative z-10">
        ...
        <button
          ...
          className="text-[11px] text-indigo-300 hover:text-indigo-200 font-bold flex items-center space-x-1 ml-1 transition-colors px-2 py-1 rounded-lg bg-indigo-500/10 hover:bg-indigo-500/20 border border-indigo-500/20"
        >
      ...
      {showTranscript && (
        <div className="mt-3 p-3.5 rounded-2xl bg-slate-950/90 border border-indigo-500/30 text-xs text-slate-300 leading-relaxed animate-in fade-in slide-in-from-top-1 duration-200 space-y-1.5">
          <div className="flex items-center space-x-1.5 text-indigo-400 text-[11px] font-bold uppercase tracking-wider">
